...
async function assign(userId: number) {
    const driveFile = await db.driveFile.create({
        data: {
            driveFileId: `${TEST_MARKER}_FILE_${Date.now()}`,
    ...
    const match = await db.parsedMatch.create({
        data: {
            matchKey: `${TEST_MARKER}_${Date.now()}`,
            contentKey: `${TEST_MARKER}_content_${Date.now()}`,
            macAdi: "TEST A vs TEST B",
    ...
    console.log(`Test maçı oluşturuldu: matchId=${match.id}, driveFileId=${driveFile.id}`);
...
async function main() {
    ...
    if (!command || !["assign", "cancel", "cleanup"].includes(command)) {
        console.error("Kullanım: TEST_USER_ID=<id> npx ts-node scripts/test-notification-flow.ts <assign|cancel|cleanup>");
        process.exit(1);
    }
